public class PersonalDataValidator {
    ...
    */
    static boolean isValid(String str) {
        ...
        // проверяем наличия нужных данных в строке
        if (splitStr.length != 3) {
            return false;
        }

        // создаем переменные для хранения переменных паспорта и СНИЛС и убираем пробелы слева и справа
        ...
        return passport.matches(passportRegEx)
                && snils.matches(snilsRegEx)
                && (country.matches(countryRegEx)
                && city.matches(cityRegEx)
                && street.matches(streetRegEx)
                && house.matches(houseRegEx)
                && apartment.matches(apartmentRegEx));
    }
}
